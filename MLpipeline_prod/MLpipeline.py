...
class ohlc_models (ohlc_transformations) :
    ...
    def fx_status_function(self, fx_status_info, lot_size, status, symbol, net_change):
        ...
        if net_change:
             fx_status_info[symbol] = {"action": action, "lot_size": lot_size, "net_change": int(net_change) }

        else:fx_status_info[symbol] = {"action": action, "lot_size": lot_size } #, 'prediction_status': status }
...
class mlpipeline(access_resources):
    ...
    def return_fx_prediction_status(cls):
        ...
        for symbol in resource.forex_pairs:
            ...
            prev_status, _, prev_date, prev_prediction_outcome = resource.previous_day_prediction(symbol, day_data, hour4_data, day=2)

...
*  ✔️ harmonic mean low experiences a fall 📉 due to negative values inclusion as prev_low - current_low `prev_low < current low` as current low value greater than previous low value causing negative difference we subtract `prev_low - current_low` small number from a larger number .
...
*  ✔️ harmonic mean low experiences a rise 📈 due to positive values inclusion as prev_low - current_low `prev_low > current low` as current low value smaller than previous low value causing positive difference we subtract large number `prev_low - current_low` from a smaller number .
  
*  ✔️ harmonic mean high experiences a fall 📉 due to negative values inclusion as current high - prev_high `current high < prev_high ` as current high value smaller than previous high value causing negative difference we subtract `current high - prev_high` small number from a larger number .
